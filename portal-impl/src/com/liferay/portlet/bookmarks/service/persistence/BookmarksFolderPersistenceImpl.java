...
public class BookmarksFolderPersistenceImpl extends BasePersistenceImpl<BookmarksFolder>
	...
	public static final FinderPath FINDER_PATH_WITH_PAGINATION_COUNT_BY_G_P_S = new FinderPath(BookmarksFolderModelImpl.ENTITY_CACHE_ENABLED,
			BookmarksFolderModelImpl.FINDER_CACHE_ENABLED, Long.class,
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public List<BookmarksFolder> findByG_P_S(long groupId,
		long[] parentFolderIds, int status) throws SystemException {
		return findByG_P_S(groupId, parentFolderIds, status, QueryUtil.ALL_POS,
			QueryUtil.ALL_POS, null);
	}
	 ...
	 */
	public List<BookmarksFolder> findByG_P_S(long groupId,
		long[] parentFolderIds, int status, int start, int end)
		throws SystemException {
		return findByG_P_S(groupId, parentFolderIds, status, start, end, null);
	}
	...
	public List<BookmarksFolder> findByG_P_S(long groupId,
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
				(orderByComparator == null)) {
			finderArgs = new Object[] {
					groupId, StringUtil.merge(parentFolderIds), status
				};
		}
		else {
			finderArgs = new Object[] {
		...
		if ((list != null) && !list.isEmpty()) {
			for (BookmarksFolder bookmarksFolder : list) {
				if ((groupId != bookmarksFolder.getGroupId()) ||
						!ArrayUtil.contains(parentFolderIds,
		...
		if (list == null) {
			...
			if ((parentFolderIds == null) || (parentFolderIds.length > 0)) {
				...
				for (int i = 0; i < parentFolderIds.length; i++) {
					query.append(_FINDER_COLUMN_G_P_S_PARENTFOLDERID_5);

					if ((i + 1) < parentFolderIds.length) {
	...
	public List<BookmarksFolder> filterFindByG_P_S(long groupId,
		long[] parentFolderIds, int status) throws SystemException {
		return filterFindByG_P_S(groupId, parentFolderIds, status,
			QueryUtil.ALL_POS, QueryUtil.ALL_POS, null);
	}
	...
	public List<BookmarksFolder> filterFindByG_P_S(long groupId,
		long[] parentFolderIds, int status, int start, int end)
		throws SystemException {
		return filterFindByG_P_S(groupId, parentFolderIds, status, start, end,
			null);
	}
	...
	public List<BookmarksFolder> filterFindByG_P_S(long groupId,
		...
		OrderByComparator orderByComparator) throws SystemException {
		if (!InlineSQLHelperUtil.isEnabled(groupId)) {
			return findByG_P_S(groupId, parentFolderIds, status, start, end,
				orderByComparator);
		}
	...
	public int filterCountByG_P_S(long groupId, long[] parentFolderIds,
		int status) throws SystemException {
		if (!InlineSQLHelperUtil.isEnabled(groupId)) {
			return countByG_P_S(groupId, parentFolderIds, status);
		}
	...
	private static final String _FINDER_COLUMN_G_P_S_PARENTFOLDERID_5 = "(" +
		_removeConjunction(_FINDER_COLUMN_G_P_S_PARENTFOLDERID_2) + ")";
