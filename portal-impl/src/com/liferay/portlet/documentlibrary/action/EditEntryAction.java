...
public class EditEntryAction extends PortletAction {
	...
	protected void restoreEntries(ActionRequest actionRequest)
		...
		long[] trashedFolderIds = StringUtil.split(
				ParamUtil.getString(actionRequest, "trashedFolderIds"), 0L);

		long[] trashedFileShortcutIds = StringUtil.split(
				ParamUtil.getString(
					actionRequest, "trashedFileShortcutIds"), 0L);

		long[] trashedFileEntryIds = StringUtil.split(
				ParamUtil.getString(actionRequest, "trashedFileEntryIds"), 0L);

		for (long deletedFolderId : trashedFolderIds) {
			DLAppServiceUtil.restoreFolderFromTrash(deletedFolderId);
		}

		for (long deletedFileShortcutId : trashedFileShortcutIds) {
			DLAppServiceUtil.restoreFileShortcutFromTrash(
				deletedFileShortcutId);
		}

		for (long deletedFileEntryId : trashedFileEntryIds) {
			DLAppServiceUtil.restoreFileEntryFromTrash(deletedFileEntryId);
		}
	}
